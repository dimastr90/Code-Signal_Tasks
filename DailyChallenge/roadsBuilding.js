...
function roadsBuilding(cities, roads) {
  let set =new Set();
  let arr = [];
  for(let i=0, end=cities;i<end; i++){
    for(let j=1+i; j<end;j++){
     set.add(JSON.stringify([i,j].sort((a,b)=>a-b)));
    }
  }
  for(let i=0, end=roads.length; i<end; i++){
    roads[i] = JSON.stringify(roads[i].sort((a,b)=>a-b));
  }
  roads = new Set(roads);
  console.log([...roads])
   console.log([...set])
